class Game{
	...
	next_day(){
		// process map events and player-related accounting
		this.map.explode();
		this.player.calculate_expenses();
		this.map.dirt_falls();
		this.map.dirt_collapses();
		this.map.check_gravity();
		this.player.player_at.x = Config.start_x;
		this.player.player_at.y = Config.start_y;
