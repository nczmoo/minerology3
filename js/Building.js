class Building{
	...
	build (from_x, from_y, to_x, to_y, what){		
		...
		while(true){
			...
			this.count ++;
			game.money -= cost;
			console.log(game.money, this.count, what);        
			pos_x += delta.x;
			pos_y += delta.y;
	...
	can_i_place(x, y, what){
		if (game.buying != 'dynamite' && game.map.grid[x][y] != 'empty'){
			return false;
		}

		if (what == 'ladder' && (y == Config.dirt_starts_at || game.map.grid[x][y - 1] != 'empty' 
			|| (game.map.is_pos_valid(x, y + 1) && game.map.grid[x][y + 1] != 'empty') || y == Config.max_y - 1)){
				return true;
		} else if (what == 'shoring' && (!game.are_they_falling(x, y))){
			return true;
		} else if (what == 'dynamite' && game.map.grid[x][y] != 'empty' && game.map.grid[x][y] != 'sky' 
